...
// ✅ Improved MBA detection logic
function isMbaCourse(data: any): boolean {
  ...
  const isMbaKeyword = mbaKeywords.some((term) => categoryText.includes(term));

  // Exclude UG/BBA
  const excludeKeywords = ["bba", "bachelor", "undergraduate", "ug"];
  const isExcluded = excludeKeywords.some(
    (term) => categoryText.includes(term) || levelText.includes(term)
  );

  return isMbaKeyword && isPgLevel && !isExcluded;
}
...
export default function MBAExplorerPage() {
  return (
    <Suspense fallback={<PageSkeleton />}>
      <MBAExplorerPageContent />
    </Suspense>
  );
}
...
function MBAExplorerPageContent() {
  ...
  const handleSelectState = (state: string) => {
    ...
    router.replace(`/course/mba.html${params.toString() ? `?${params.toString()}` : ""}`, { scroll: true });
  };
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <button
              ...
              onClick={() => {
                ...
                setSelectedState("");
                setSelectedCountry("");
                router.replace("/course/mba.html", { scroll: true });
              }}
        ...
        <div className="bg-white border rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
            <h2 className="text-lg font-semibold text-gray-800">
              {selectedState
                ? `MBA Colleges in ${selectedState}`
                : selectedCountry
                ? `MBA Colleges in ${selectedCountry}`
                : "All MBA Colleges"}
            </h2>
